...
class ImageProcessor(Node):
    def __init__(self, arc, run_name, use_gpu=False, threshold=0.5, dehaze_t0=0.6, dehaze_atmos_factor=0.8, color_diff_scale=0.5,
        ...
        # Depth processing setup - metric depth in metres (0..max_depth)
        self.depth_processor = DepthAnythingProcessor(encoder='vits', device=self.device, metric=True, max_depth=20.0)
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            # Draw obstacles on frame (after depth inference so depth is on clean frame)
            ...
            # Publish depth map — normalize metric values (0..max_depth metres) to 0-255 for display
            depth_uint8 = np.clip(depth_np / 20.0 * 255, 0, 255).astype(np.uint8)
